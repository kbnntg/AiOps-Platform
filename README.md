...
基于 Kubernetes 的云原生智能运维平台，集 **节点监控、SRE 级告警治理、AI 智能诊断、Pod 生命周期管理、操作审计** 于一体。

> 从单机 Python 脚本演进而来，完成 **容器化 → K8s 部署 → Web 平台化 → AI 智能化 → 告警治理** 的完整升级。
...
- **滑动窗口告警**：N 个采样点超标 M% 才触发，过滤抖动误报
- **告警聚合**：窗口内多条告警合并，AI 推断共同根因
- **参数可配**：窗口大小、超标比例、冷却时间通过环境变量调整
...
1. 环境检测（Docker / K8s / 磁盘空间）
2. 输入配置（MySQL 密码、AI API Key、Webhook）
3. 自动构建三个镜像
4. 创建命名空间和 Secret
5. 依次部署 MySQL → 采集器 → 后端 → 前端 → Prometheus
6. 健康检查并输出访问地址
...
**问题**：CPU 在 80% 附近抖动时频繁告警。

**解决**：改用滑动窗口——10 个采样点中超过阈值的占比达 70% 才告警，过滤瞬时尖峰。

### 5. 多节点告警风暴

**问题**：多节点同时告警时，运维被刷屏，难以关联。

**解决**：实现告警聚合——5 分钟窗口内的告警进入缓冲区，窗口到期后批量交给 AI 分析共同根因，生成一条聚合告警。
...
| `INTERVAL`              | 采集间隔（秒）           | 3             |
| `ALERT_WINDOW_SIZE`     | 滑动窗口大小（采样点数） | 10            |
| `ALERT_THRESHOLD_RATIO` | 滑动窗口超标占比阈值     | 0.7           |
| `ALERT_COOLDOWN`        | 告警后冷却时间（秒）     | 60            |
...
| 场景           | WINDOW_SIZE | THRESHOLD_RATIO | AGGREGATE_WINDOW |
| :------------- | :---------- | :-------------- | :--------------- |
| 严格（易告警） | 5           | 0.6             | 120              |
| 平衡（默认）   | 10          | 0.7             | 300              |
| 宽松（少告警） | 20          | 0.8             | 600              |
...
| 变化                                | 说明                               |
| :---------------------------------- | :--------------------------------- |
| **新增"SRE 级告警治理"章节**        | 突出完整链路：滑动窗口 → 聚合 → AI |
| **功能特性合并告警治理**            | 加入聚合能力说明                   |
| **技术栈新增"告警治理"**            | 滑动窗口、冷却、聚合、AI           |
| **项目结构新增 `aggregation.py`**   | 明确聚合器模块                     |
| **技术难点新增 2 条**               | 固定阈值误报 + 告警风暴            |
| **环境变量新增 `AGGREGATE_WINDOW`** | 聚合窗口配置                       |
| **新增"告警治理演进路线"**          | 4 阶段图示                         |
| **项目数据更新**                    | AI 能力从 3 类到 4 类              |
| **新增 2 张截图**                   | 滑动窗口 + 告警聚合                |
- [DeepSeek](https://www.deepseek.com/)
