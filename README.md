...
| 阶段 | 方案                                     | 效果                         |
| :--- | :--------------------------------------- | :--------------------------- |
| 初版 | 固定阈值 + 冷却时间                      | 抖动型负载频繁误报           |
| 升级 | **滑动窗口（N 个采样点超标 M% 才告警）** | 区分"瞬时尖峰"和"持续高负载" |

**实测效果**：窗口大小 10 个点、超标占比 70%，5 秒一次的抖动不会触发告警；持续 30 秒的高负载才会告警。
